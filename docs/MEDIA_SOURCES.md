...
| [Yarn/GetYarn](https://getyarn.io/) | Implemented link-only movie/TV quote discovery and possible commercial partner | No public developer API or reusable-content license was located in the 2026-08-30 review. The current HTML and media hosts return Cloudflare browser challenges to backend clients. The Go adapter constructs official search/clip pages but does not scrape or infer CDN downloads. | Store no Yarn media. Open the provider page in the user's browser; a richer connector requires written API/content permission. |
...
8. Keep the implemented Yarn option link-only, and contact Yarn about an API/content partnership before adding in-app results, previews, downloads, transformations, or storage.
...
GoGIF's `internal/provider/yarn` package is a clean Go implementation of the safe portion of the behavior: query normalization, exact UUID/URL validation, official page construction, conservative rights metadata, and a link-only handling policy. It performs no network request. Do not add browser-cookie forwarding, challenge solving, `curl_cffi`, CDN filename derivation, or media proxying as a substitute for an API agreement.
