# Media sources and ingestion policy
...
| [GIPHY Clips](https://developers.giphy.com/docs/clips/) | Short clips with sound from official partners | Clip endpoints require prior approval. Use a separate source section and platform key. | Provider-hosted unless a separate agreement grants ingestion rights. |
...
| Yarn/GetYarn | Product inspiration and possible commercial partner | No public developer API or reusable-content license was located in the 2026-08-29 review. Do not scrape it. | No ingestion until Yarn grants explicit API and content rights in writing. |
...
- allowed storage mode: `external`, `selected-import`, or `managed`;
...
1. Keep the existing GIPHY integration and formalize it behind a provider adapter.
2. Apply for GIPHY Clips and KLIPY production access in parallel.
3. Build Wikimedia Commons ingestion first for the reusable open-media catalog.
4. Add allowlisted Internet Archive/Prelinger and NASA collections.
5. Add Pixabay/Pexels only as user-directed editor inputs after a terms review.
6. Contact Yarn about an API/content partnership; do not block the MVP on it.
