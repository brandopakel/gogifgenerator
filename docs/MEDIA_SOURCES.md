...
| [Clip.Cafe API](https://github.com/ClipCafe/clipcafe) | Closest documented API match for quote-first movie/TV cards, transcript search, metadata, pagination, and clip downloads | Requires a paid API subscription. Its API documentation is not itself a commercial content license; obtain written confirmation that GoGIF's display, transformation, and paid-product use are permitted before integration. | Provider-hosted/link-only until commercial and derivative rights are confirmed in writing. |
...
| [Yarn/GetYarn](https://getyarn.io/) | Experimental movie/TV quote metadata discovery and possible commercial partner | No public developer API or reusable-content license was located in the 2026-08-30 review. The live HTML host currently returns a Cloudflare browser challenge to backend clients. The Go adapter parses bounded public result metadata when available and constructs only official page/embed URLs. | Store no Yarn media. Fail closed on challenges; production reliability or transformation requires written API/content permission. |
...
8. Keep the experimental Yarn option metadata/embed-only, and contact Yarn about an API/content partnership before relying on it for production search or adding downloads, transformations, or storage.

## Yarn implementation review

Two community implementations informed the compatibility review but are not embedded dependencies:

- [`cbrochtrup/getgetyarnio`](https://github.com/cbrochtrup/getgetyarnio) is an Apache-2.0 Bash/FFmpeg utility that parses the old `/yarn-find` HTML and derives `y.yarn.co` MP4 paths.
- [`lambdan/getyarn_scraper.py`](https://gist.github.com/lambdan/5c6c49b88f9d5ed72ccc955e879bf402) is a Python/BeautifulSoup downloader. No license grant is stated on the gist page, so its source must not be copied.
- [`gaardhus/getyearn`](https://github.com/gaardhus/getyearn) and [`alex0112/twine`](https://github.com/alex0112/twine) show the same known-URL/CDN convention in Python and Rust respectively; they do not provide a supported Yarn API.
- [`zhangzhanglaila/AI-Montage-Agent`](https://github.com/zhangzhanglaila/AI-Montage-Agent) disables its Yarn crawler because the current site is Cloudflare-blocked.

GoGIF's `internal/provider/yarn` package is a clean Go implementation of the bounded metadata portion of the behavior: phrase-query normalization, one-at-a-time HTTP fetching with a 15-second timeout and 6 MiB response ceiling, HTML result-card parsing, canonical UUID validation, conservative pagination, official page/embed construction, and unknown-rights metadata. Fixture tests cover extraction, deduplication, pagination, malicious-host rejection, challenge handling, and the no-media-download boundary. The adapter returns `provider.ErrUnavailable` when Yarn sends a challenge. Do not add browser-cookie forwarding, challenge solving, `curl_cffi`, CDN filename derivation, media downloading, or proxying as a substitute for an API agreement.
